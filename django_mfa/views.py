...
from django_mfa.models import is_mfa_enabled, UserOTP, UserRecoveryCodes
...
def recovery_codes(request):
    if request.method == "GET":
        if UserOTP.objects.filter(user=request.user.id).exists():
            ...
            # print(codes,"codes")
            # user_codes=UserRecoveryCodes.objects.filter(user=UserOTP.objects.get(user=request.user.id))
...
def recovery_codes_download(request):
    ...
    codes_list = [i.secret_code for i in UserRecoveryCodes.objects.filter(
        user=UserOTP.objects.get(user=user_id))]
