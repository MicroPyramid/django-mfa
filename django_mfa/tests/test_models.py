...
class Test_Models_Mfa(TestCase):
    def setUp(self):
        ...
        self.user_codes = UserRecoveryCodes.objects.create(
            user=UserOTP.objects.get(user=self.user), secret_code="secret_code"
        )
    ...
    def test_userotp_data_saved_correctly(self):
        user_otp = UserOTP.objects.filter(user=auth.get_user(self.client)).first()
